...
function setupApp () {
    ...
    app.get('/testgroupby', (req, res) => {
      var data=[ 
        ...
        var result = alasql('SELECT category, sum(hits) AS hits,MONTH(date)  as month,YEAR(date) as year \
        FROM ? \
        GROUP BY MONTH(date),YEAR(date) \
        ORDER BY year DESC',[data]);
    ...
    app.get("/generate_ndiagpermont",(req, res)=>{
      let newlyDiagnosesMeta=importConfig.dataElements2Generate.find(
        ...
        getEventAnalytics(eventQuery,function(result){
          res.send(result);
