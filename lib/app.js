...
function setupApp () {
    ...
    app.get('/testgroupby', (req, res) => {
      /*var data=[ 
        ...
        var data=[{"uuid":"id0001","sex":"","visite":"2020-01-02"},
                {"uuid":"id0001","sex":"","visite":"2020-02-02"},
                {"uuid":"id0001","sex":"M","visite":"2020-03-03"},
                {"uuid":"id0002","sex":"F","visite":"2020-01-02"},
                {"uuid":"id0002","sex":"F","visite":"2020-02-02"},
                {"uuid":"id0002","sex":"F","visite":"2020-04-03"},
                {"uuid":"id0003","sex":"M","visite":"2020-07-02"},
        ...
        var result = alasql('SELECT uuid,sex,visite \
        FROM ? \
        where sex <> ""',[data]);
    ...
    app.get("/generate_newlytreatpermonth",(req, res)=>{
      let newlyDiagnosesMeta=importConfig.dataElements2Generate.find(
        ...
        let listEventRecords = alasql('SELECT uuid,ageGroup,sex,MIN(dateVisite) as dateVisite \
        FROM ? \
        GROUP BY uuid,sex,ageGroup ',[listEventElementsWithTraitement]);
    ...
    app.get("/generate_patientfollowupmonth",(req, res)=>{
      let newlyDiagnosesMeta=importConfig.dataElements2Generate.find(
        ...
        FROM ? \
        GROUP BY uuid,sex,ageGroup ',[listEventElements]);*/
        let listEventRecordsGrouped = alasql('SELECT COUNT(uuid) as nb,ageGroup,sex,dateVisite \
        FROM ? \
        GROUP BY dateDiagnosed,sex,ageGroup ',[listEventElements]);
        //now count rows group by agegrou and sex
