...
function setupApp () {
    ...
    app.get('/testgroupby', (req, res) => {
      /*var data=[ 
        ...
        var data=[{"uuid":"id0001","sex":"M","visite":"2020-01-02"},
                {"uuid":"id0001","sex":"M","visite":"2020-02-02"},
                {"uuid":"id0001","sex":"M","visite":"2020-02-03"},
                {"uuid":"id0002","sex":"F","visite":"2020-01-02"},
                {"uuid":"id0002","sex":"F","visite":"2020-02-02"},
                {"uuid":"id0002","sex":"F","visite":"2020-02-03"},
                {"uuid":"id0003","sex":"M","visite":"2020-03-02"},
                {"uuid":"id0003","sex":"M","visite":"2020-05-02"},
                {"uuid":"id0003","sex":"M","visite":"2020-04-03"}
      ]
         /*var result = alasql('SELECT category, sum(hits) AS hits, count(bytes) as bytes \
        FROM ? \
        GROUP BY category \
        ORDER BY bytes DESC',[data]);*/
        var result = alasql('SELECT uuid,sex,MIN(visite) \
        FROM ? \
        GROUP BY uuid,sex',[data]);
      ...
      getListOrgUnit(dhis2Token,function(listOrgUnits){
        ...
        getListTrackedEntities(dhis2Token,listOrgUnits,function(listTEIs)
        {
          ...
          if(listTEIs.length>0)
          {
            ...
            generateValueEndPoint(listTEIs.length,function(listGeneratedValues){
    ...
    app.get("/generate_ndiagpermont",(req, res)=>{
      let newlyDiagnosesMeta=importConfig.dataElements2Generate.find(
        ...
        let listEventRecords = alasql('SELECT COUNT(_index) as nb,dateDiagnosed,sex,ageGroup \
        FROM ? \
